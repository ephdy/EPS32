...
void oled_witre_char(uint8_t Page,uint8_t Coul,uint8_t chr,uint8_t width){
    ...
    case 6://字符宽度6
        DC_Command;
        oled_set_cursor(Page,Coul);
        DC_Data;
        for (uint8_t i = 0; i < 6; i++)
        {
            oled_write_byte(oled_f8x6[chr-' '][i]);
        }
        break;
	case 8://字符宽度8
        DC_Command;
        ...
        for (uint8_t i = 0; i < 8; i++)
        {
            oled_write_byte(oled_f16x8[(chr-' ')*2][i]);
        }
        DC_Command;
        oled_set_cursor(Page+1,Coul);
        DC_Data;
        for (uint8_t i = 0; i < 8; i++)
        {
            oled_write_byte(oled_f16x8[(chr-' ')*2+1][i]);
        }
        break;
